...
public class GridBagLayout implements LayoutManager2,
  ...
  public Point location(int x, int y) {
    ...
    d = layoutInfo.startx;
    if (!rightToLeft) {
	//FD        for (i=0; i<layoutInfo.width; i++) {
        i=0;
        for (; i<layoutInfo.width; i++) {
	    d += layoutInfo.minWidth[i];
	    if (d > x)
		break;
	}
    } else {
	//FD        for (i=layoutInfo.width-1; i>=0; i--) {
        i=layoutInfo.width-1;
        for (; i>=0; i--) {
	    if (d > x)
		break;
	    d += layoutInfo.minWidth[i];
    ...
    d = layoutInfo.starty;
    //FD    for (i=0; i<layoutInfo.height; i++) {
    i=0;
    for (; i<layoutInfo.height; i++) {
  ...
  protected GridBagLayoutInfo GetLayoutInfo(Container parent, int sizeflag) {
   synchronized (parent.getTreeLock()) {
    ...
    //FD    for (compindex = 0 ; compindex < components.length ; compindex++) {
    compindex = 0 ;
    for ( ; compindex < components.length ; compindex++) {
	...
	py = 0;
	//FD	for (i = curX; i < (curX + curWidth); i++)
	i = curX;
	for (; i < (curX + curWidth); i++)
	...
	if(curY < 0)
      ...
      //FD      for (px = curX + curWidth; r.width < px; r.width++);
      px = curX + curWidth;
      for (; r.width < px; r.width++);
      //FD      for (py = curY + curHeight; r.height < py; r.height++);
      py = curY + curHeight;
      for (; r.height < py; r.height++);

      /* Adjust the xMax and yMax arrays */
      //FD      for (i = curX; i < (curX + curWidth); i++) { yMax[i] = py; }
      i = curX;
      for (; i < (curX + curWidth); i++) { yMax[i] = py; }
      //FD      for (i = curY; i < (curY + curHeight); i++) { xMax[i] = px; }
      i = curY;
      for (; i < (curY + curHeight); i++) { xMax[i] = px; }
	...
	px = 0;
	//FD	for (i = curY; i < (curY + curHeight); i++)
	i = curY;
	for (; i < (curY + curHeight); i++)
	  px = Math.max(px, xMax[i]);
	...
	if (constraints.tempWidth == i) {
	  ...
	  //FD	  for (k = constraints.tempX; k < px; k++)
	  k = constraints.tempX;
	  for (; k < px; k++)
	  ...
	  if (weight_diff > 0.0) {
	    ...
	    for (; k < px; k++)
	      weight += r.weightX[k];
	    //FD	    for (k = constraints.tempX; weight > 0.0 && k < px; k++) {
	    k = constraints.tempX;
	    for (; weight > 0.0 && k < px; k++) {
	...
	if (constraints.tempHeight == i) {
	  ...
	  //FD	  for (k = constraints.tempY; k < py; k++)
	  k = constraints.tempY;
	  for (; k < py; k++)
	  ...
	  if (weight_diff > 0.0) {
	    ...
	    for (; k < py; k++)
	      weight += r.weightY[k];
	    //FD	    for (k = constraints.tempY; weight > 0.0 && k < py; k++) {
	    k = constraints.tempY;
	    for (; weight > 0.0 && k < py; k++) {
  ...
  protected void ArrangeGrid(Container parent) {
	...
	continue;
      ...
      if (!rightToLeft) {
        r.x = info.startx;
	//FD        for(i = 0; i < constraints.tempX; i++)
        i = 0;
        for(; i < constraints.tempX; i++)
	  r.x += info.minWidth[i];
      } else {
	...
	for(; i < constraints.tempX; i++)
      ...
      r.y = info.starty;
      //FD      for(i = 0; i < constraints.tempY; i++)
      i = 0;
      for(; i < constraints.tempY; i++)
	r.y += info.minHeight[i];

      r.width = 0;
      //FD      for(i = constraints.tempX;
      i = constraints.tempX;
      for(;
