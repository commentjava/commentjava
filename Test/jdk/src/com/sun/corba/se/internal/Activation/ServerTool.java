...
public class ServerTool
{
	...
	try {
	    Repository rep = RepositoryHelper.narrow( 
				     orb.resolve_initial_references( "ServerRepository" ) ) ;
	...
	try {
	    ...
	    props.put("org.omg.CORBA.ORBClass", ORBConstants.IIOP_ORB_NAME);
...
class RegisterServer implements CommandHandler
{
	...
	try {
	    ...
	    Activator activator = ActivatorHelper.narrow(
					 orb.resolve_initial_references("ServerActivator"));
...
class LocateServer implements CommandHandler
{
	...
	try {
	    ...
	    Locator locator = LocatorHelper.narrow(
					   orb.resolve_initial_references("ServerLocator"));
